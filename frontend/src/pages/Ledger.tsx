...
import {
  Receipt, Search, CheckCircle2, XCircle,
  Download, FileText, ChevronLeft, ChevronRight,
  DollarSign, ArrowUpDown, X, Filter, SearchX, Inbox,
  Calendar,
} from 'lucide-react';
...
export const Ledger: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg bg-[var(--bg-card)] p-6 shadow-[0_1px_3px_0_rgba(0,0,0,0.3)] space-y-5">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          {hasAnyFilter && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-[var(--text-muted)] hover:text-rose-400 hover:bg-rose-950/10 transition-all duration-200 cursor-pointer"
            >
      ...
      <div className="rounded-lg bg-[var(--bg-card)] p-6 shadow-[0_1px_3px_0_rgba(0,0,0,0.3)]">
        <h2 className="text-sm font-semibold text-[var(--text-primary)] mb-6 flex items-center gap-2">
          <Receipt className="h-4 w-4 text-[var(--accent)]" /> Ledger Records
          <span className="ml-auto text-[11px] text-[var(--text-muted)] font-normal">
            {processedTransactions.length} {processedTransactions.length === 1 ? 'entry' : 'entries'}
          </span>
        </h2>
              ...
              <table className="w-full text-left text-xs text-[var(--text-secondary)] border-collapse min-w-[700px]">
                <thead>
                  <tr className="text-[var(--text-muted)]">
                    <th className="pb-3 pr-2 font-medium text-[10px] uppercase tracking-wider whitespace-nowrap">ID</th>
                    <th className="pb-3 pr-2 font-medium text-[10px] uppercase tracking-wider whitespace-nowrap">Sender VPA</th>
                    <th className="pb-3 pr-2 font-medium text-[10px] uppercase tracking-wider whitespace-nowrap">Receiver VPA</th>
                    <th className="pb-3 pr-2 font-medium text-[10px] uppercase tracking-wider whitespace-nowrap">Amount</th>
                    <th className="pb-3 pr-2 font-medium text-[10px] uppercase tracking-wider whitespace-nowrap">Hops</th>
                    <th className="pb-3 pr-2 font-medium text-[10px] uppercase tracking-wider whitespace-nowrap">Settled</th>
                    <th className="pb-3 pr-2 font-medium text-[10px] uppercase tracking-wider whitespace-nowrap">Status</th>
                    <th className="pb-3 text-right font-medium text-[10px] uppercase tracking-wider whitespace-nowrap">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {paginatedTransactions.map((tx) => (
                    <tr
                      key={tx.id}
                      className="border-t border-[var(--border)] hover:bg-[var(--bg-subtle)] transition-colors duration-150"
                    >
                      <td className="py-4 pr-2 text-[var(--accent)] font-medium whitespace-nowrap">#{tx.id}</td>
                      <td className="py-4 pr-2 font-medium text-[var(--text-primary)] break-all max-w-[140px]">{tx.senderVpa}</td>
                      <td className="py-4 pr-2 font-medium text-[var(--text-primary)] break-all max-w-[140px]">{tx.receiverVpa}</td>
                      <td className="py-4 pr-2 font-medium text-[var(--text-primary)] whitespace-nowrap">₹{tx.amount.toFixed(2)}</td>
                      ...
                      <td className="py-4 pr-2 text-[var(--text-muted)] text-[10px] whitespace-nowrap">
                      ...
                      <td className="py-4 pr-2 whitespace-nowrap">
                        ...
                        >
                          {tx.status === 'SETTLED' ? <CheckCircle2 className="w-3 h-3" /> : <XCircle className="w-3 h-3" />}
                          {tx.status}
                        </span>
                      </td>
                      <td className="py-4 text-right whitespace-nowrap">
                        <button
                          ...
                          className="rounded-lg border border-[var(--border)] px-3 py-1.5 text-[10px] font-medium text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-card-alt)] transition-all duration-200 cursor-pointer"
                        >
                          Inspect
                        </button>
                      </td>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6 pt-4 border-t border-[var(--border)]">
              <div className="flex items-center gap-2">
                ...
                <Select
                  ...
                  className="w-auto min-w-[70px] py-1.5"
                />
              </div>
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="flex items-center gap-1 rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs font-medium text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-subtle)] disabled:opacity-40 disabled:cursor-not-allowed transition-all duration-200 cursor-pointer"
                >
